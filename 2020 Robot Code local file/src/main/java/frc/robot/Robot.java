...
public class Robot extends TimedRobot {
  ...
  private final Solenoid cellRetract = new Solenoid(0);
  private final Solenoid cellExtract = new Solenoid(1);
  ...
  public void teleopPeriodic() {
    ...
    m_drive.arcadeDrive(m_stick.getY()*-1, m_stick.getX());

    //Pneumatic piston command goes here
    if(m_stick.getRawButton(1) == true) {
      cellRetract.set(false);
      cellExtract.set(true);
    } else {
      cellRetract.set(true);
      cellExtract.set(false);
    }
